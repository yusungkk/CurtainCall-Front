...
const PaymentPage = () => {
    ...
    useEffect(() => {
        alert("*** 주의: 결제하기 버튼을 누르신 후 5분 안에 결제가 완료되지 않으면 결제가 자동으로 취소되니 유의 바랍니다. ***\n(선택하신 좌석도 해제됩니다.)");
    }, []);

    useEffect(() => {
        if (isPaymentStarted && timeLeft !== null) { // 결제가 시작된 경우에만 타이머 작동
    ...
    const initiatePayment = (orderId) => {
        const { IMP } = window;
        IMP.init("imp54038810"); // 포트원 가맹점 식별 코드
