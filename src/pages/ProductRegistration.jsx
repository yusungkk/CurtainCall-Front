            ...
            <div className="form-box">
                ...
                <form onSubmit={handleSubmit}>
                    ...
                    <div className="input-group">
                        <label>상품명</label>
                        <input type="text" placeholder="상품명" value={productName} onChange={(e) => setProductName(e.target.value)} />
                    </div>
                    ...
                    <div className="input-group">
                        <label>장소</label>
                        <input type="text" placeholder="장소" value={place} onChange={(e) => setPlace(e.target.value)} />
                    </div>
                    ...
                    <div className="input-group">
                        <label>러닝타임 (분)</label>
                        <input type="number" placeholder="러닝타임 (분)" value={runningTime} onChange={(e) => setRunningTime(e.target.value)} />
                    </div>
                    {errors.runningTime && <p className="error">{errors.runningTime}</p>}

                    <div className="input-group">
                        <label>가격 (원)</label>
                        <input type="number" placeholder="가격 (원)" value={price} onChange={(e) => setPrice(e.target.value)} />
                    </div>
                    ...
                    <div className="input-group">
                        <label>공지사항</label>
                        <textarea placeholder="공지사항(500자 내외)" value={notice} onChange={(e) => setNotice(e.target.value)} />
                    </div>
                    ...
                    {productDetails.map((detail, index) => (
                        <div key={index} className="schedule-group">
                            ...
                            <button type="button" className="remove-btn" onClick={() => removeProductDetail(index)}>❌ </button>
                        </div>
                    ))}
                    {errors.productDetails && <p className="error">{errors.productDetails}</p>}

                    <button type="button" onClick={addProductDetail} className="add-btn">+ 일정 추가</button>
